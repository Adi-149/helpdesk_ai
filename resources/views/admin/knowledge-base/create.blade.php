<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Tambah Knowledge Baru</h2>
                <p class="text-sm text-gray-500 mt-1">Tambahkan pengetahuan baru ke database internal</p>
            </div>
    ...
    <div class="max-w-3xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        <div class="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
            <div class="px-6 py-5 border-b border-gray-200 bg-gray-50">
                <h3 class="text-sm font-bold text-gray-700 uppercase tracking-wider">Formulir Data Knowledge</h3>
            </div>
            ...
            <form method="POST" action="{{ route('admin.knowledge-base.store') }}" class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title') }}" required
                        ...
                        placeholder="Contoh: WiFi Kantin Putri">
                    @error('title')
                ...
                <div>
                    ...
                    <input type="text" name="keywords" id="keywords" value="{{ old('keywords') }}" required
                        class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        placeholder="Contoh: wifi, kantin putri, password, internet">
                    <p class="mt-1 text-xs text-gray-500">Pisahkan dengan koma. Kata kunci membantu chatbot menemukan informasi ini.</p>
                    @error('keywords')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="8" required
                        class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        placeholder="Tulis informasi lengkap yang akan digunakan chatbot untuk menjawab pertanyaan pengguna...">{{ old('content') }}</textarea>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <select name="access_level" id="access_level" required
                            ...
                            @foreach(\App\Models\KnowledgeBase::ACCESS_LEVELS as $key => $label)
                                <option value="{{ $key }}" {{ old('access_level', 'user') === $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-xs text-gray-500">Hanya data aktif yang digunakan chatbot.</p>
                    </div>
                </div>
