    ...
    <div class="max-w-5xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white border border-gray-200 shadow-sm rounded-lg p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('tickets.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">+ Buat Tiket</a>
            </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-sm">
                                <span class="inline-flex items-center px-2 py-1 rounded text-xs font-medium">
                                    @if($ticket->assigned_to && $ticket->status === 'open')
                                        <span class="bg-purple-100 text-purple-850 px-2 py-0.5 rounded font-semibold">Ditangani</span>
                                    @elseif($ticket->status === 'open')
                                        <span class="bg-green-100 text-green-850 px-2 py-0.5 rounded font-semibold">Dibuka</span>
                                    @elseif($ticket->status === 'progress')
                                        <span class="bg-yellow-100 text-yellow-850 px-2 py-0.5 rounded font-semibold">Sedang Diproses</span>
                                    @elseif($ticket->status === 'resolved')
                                        <span class="bg-blue-100 text-blue-855 px-2 py-0.5 rounded font-semibold">Diselesaikan</span>
                                    @else
                            ...
                            <td class="px-4 py-3 text-right text-sm">
                                <a href="{{ route('tickets.show', $ticket->id) }}" class="inline-flex items-center px-3 py-1 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-xs font-semibold">Lihat</a>
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td class="px-4 py-6 text-center text-sm text-gray-500 font-medium" colspan="{{ auth()->user()->role === 'admin' ? 6 : 5 }}">Belum ada tiket.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
