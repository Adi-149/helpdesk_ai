<x-app-layout>
    ...
    <div class="{{ auth()->user()->role === 'user' ? 'max-w-4xl' : 'max-w-7xl' }} mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 {{ auth()->user()->role === 'user' ? '' : 'lg:grid-cols-3' }} gap-6">
            ...
            <div class="border-b pb-6 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <p class="text-gray-900">{{ $ticket->user ? $ticket->user->name : 'Pengguna Dihapus' }}</p>
                    </div>
                    <div>
                    ...
                    <p class="text-gray-900 mt-1">
                        <span class="inline-flex items-center px-3 py-1 rounded text-sm font-medium">
                            ...
                            @elseif($ticket->assigned_to)
                                <span class="bg-purple-100 text-purple-800 px-3 py-1 rounded">Ditangani</span>
            ...
            <!-- AI Resolution Summary (Tahap 8) -->
            @if($ticket->resolution_summary)
                <div class="mb-6 p-5 rounded-xl border border-emerald-200 bg-emerald-50/30 dark:bg-emerald-950/10 dark:border-emerald-900/40">
                    <div class="flex items-center space-x-2 mb-3 text-emerald-800 dark:text-emerald-300 font-bold text-sm">
            ...
            @if($ticket->status === 'resolved' && auth()->id() === $ticket->user_id)
                <div class="mb-6 p-4 rounded-lg bg-indigo-50 border border-indigo-200 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div>
                        <h4 class="text-sm font-semibold text-indigo-800">Konfirmasi Penyelesaian Tiket</h4>
                        <p class="text-xs text-indigo-600 mt-1">Teknisi telah menyelesaikan kendala Anda. Silakan klik tombol di samping untuk menutup tiket ini jika kendala sudah benar-benar selesai.</p>
                    </div>
                    <form method="POST" action="{{ route('tickets.close', $ticket->id) }}">
                        @csrf
                        <button type="submit" class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded text-sm font-semibold transition-colors">
                            Konfirmasi Selesai
                        </button>
                    </form>
                </div>
            ...
            @if($ticket->status === 'closed' && (auth()->id() === $ticket->user_id || auth()->user()->role === 'admin'))
                ...
                @if($canReopen)
                    <div class="mb-6 p-4 rounded-lg bg-yellow-50 border border-yellow-200 flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div>
                            <h4 class="text-sm font-semibold text-yellow-800">Apakah kendala muncul kembali?</h4>
                        ...
                        <form method="POST" action="{{ route('tickets.reopen', $ticket->id) }}">
                            @csrf
                            <button type="submit" class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 bg-yellow-600 hover:bg-yellow-700 text-white rounded text-sm font-semibold transition-colors whitespace-nowrap">
                                Buka Kembali Tiket
                            </button>
                        </form>
                    </div>
                            ...
                            <div class="border-l-4 border-indigo-400 bg-indigo-50 p-4 rounded">
                                <div class="flex items-center justify-between mb-2">
                                    ...
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <span class="text-xs bg-indigo-100 text-indigo-800 px-2 py-1 rounded">{{ $history->new_status === 'open' && $ticket->assigned_to ? 'Ditangani' : ($history->new_status === 'open' ? 'Dibuka' : ($history->new_status === 'progress' ? 'Sedang Diproses' : ($history->new_status === 'resolved' ? 'Diselesaikan' : 'Ditutup'))) }}</span>
                                    </div>
                                </div>
                                @if($history->notes)
        ...
        <div class="bg-white shadow-sm rounded-lg p-6 mt-6">
            ...
            <!-- Box Pesan Chat -->
            <div class="border rounded-lg bg-gray-50 p-4 mb-4 max-h-[400px] overflow-y-auto space-y-4">
                @forelse($ticket->messages->sortBy('created_at') as $message)
                    @php
                        ...
                        if ($senderRole === 'support') {
                            $roleBadgeColor = 'bg-indigo-100 text-indigo-800';
                    ...
                    <div class="flex {{ $isOwnMessage ? 'justify-end' : 'justify-start' }}">
                        <div class="max-w-[75%] rounded-lg px-4 py-2 {{ $isOwnMessage ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-white text-gray-800 border rounded-bl-none shadow-sm' }}">
                            @if(!$isOwnMessage)
                            ...
                            <p class="text-sm whitespace-pre-line leading-relaxed">{{ $message->message }}</p>
                            
                            <div class="text-[10px] mt-1 text-right {{ $isOwnMessage ? 'text-indigo-200' : 'text-gray-400' }}">
            ...
            <form id="chat-form" action="{{ route('tickets.messages.store', $ticket->id) }}" method="POST" class="mt-4">
                ...
                <div class="flex gap-2">
                    <textarea 
                        ...
                        name="message" 
                        rows="2" 
                        class="flex-1 border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-200 text-sm resize-none" 
                        placeholder="Tulis pesan... (Enter untuk kirim, Shift+Enter untuk baris baru)" 
                        required></textarea>
                    <button 
                        type="submit" 
                        class="px-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded text-sm font-semibold flex items-center justify-center gap-1 transition-colors">
            ...
            <script>
                ...
                function appendMessage(msg) {
                    ...
                    bubble.className = `max-w-[75%] rounded-lg px-4 py-2 ${isOwn ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-white text-gray-800 border rounded-bl-none shadow-sm'}`;
                    if (!isOwn) {
                    ...
                    timeDiv.className = `text-[10px] mt-1 text-right ${isOwn ? 'text-indigo-200' : 'text-gray-400'}`;
        ...
        <!-- Right Column: AI Assistant & Similar Cases (Tahap 5 & 6) -->
        @if(auth()->user()->role === 'support' || auth()->user()->role === 'admin')
        <div class="space-y-6">
            <!-- AI Assistant Card -->
            <div class="bg-gradient-to-br from-indigo-900 via-indigo-950 to-slate-900 rounded-xl shadow-xl border border-indigo-950 text-white p-6 relative overflow-hidden">
                ...
                <div class="flex items-center space-x-2 mb-4">
                    ...
                    <h3 class="text-xs font-extrabold uppercase tracking-widest text-indigo-200">Asisten AI Teknisi</h3>
                </div>

                <div class="space-y-4">
                    <!-- Summary -->
                    <div>
                        <span class="text-[9px] uppercase tracking-wider text-indigo-300 font-bold block">Ringkasan Masalah</span>
                        <p class="text-xs text-gray-200 mt-1 leading-relaxed">{{ $ticket->ai_summary }}</p>
                    </div>

                    <!-- Causes -->
                    @if($ticket->ai_causes)
                    <div>
                        <span class="text-[9px] uppercase tracking-wider text-indigo-300 font-bold block">Analisis Penyebab</span>
                        <div class="text-xs text-gray-200 mt-1 space-y-1 leading-relaxed whitespace-pre-line">
                            {{ $ticket->ai_causes }}
                        </div>
                    </div>
                    @endif
                    ...
                    <div>
                        <span class="text-[9px] uppercase tracking-wider text-indigo-300 font-bold block">Rekomendasi Tindakan</span>
                        <div class="text-xs text-gray-200 mt-1 space-y-1 leading-relaxed whitespace-pre-line">
                    ...
                    <!-- Confidence & Priority -->
                    <div class="grid grid-cols-2 gap-3 pt-3 border-t border-indigo-800/40">
                        <div>
                           <span class="text-[9px] uppercase tracking-wider text-indigo-300 font-bold block">Keyakinan AI</span>
                           <span class="text-xs font-bold text-emerald-400 mt-0.5 block">{{ $ticket->ai_confidence ?: '85%' }}</span>
                        </div>
                        <div>
                           <span class="text-[9px] uppercase tracking-wider text-indigo-300 font-bold block">Prioritas</span>
                           <span class="text-xs font-bold text-gray-200 mt-0.5 block uppercase {{ $ticket->priority === 'high' ? 'text-red-400' : ($ticket->priority === 'medium' ? 'text-orange-400' : 'text-green-400') }}">{{ $ticket->priority }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 p-6">
                <div class="flex items-center space-x-2 mb-4 pb-3 border-b border-gray-100 dark:border-gray-700">
                    ...
                    <h3 class="text-sm font-bold text-gray-900 dark:text-white">Kasus Serupa</h3>
                </div>

                <div class="space-y-4">
                    @forelse($similarTickets as $match)
                        <div class="p-3 bg-gray-50 dark:bg-gray-700/30 rounded-lg border border-gray-100 dark:border-gray-700 space-y-2">
                            <div class="flex items-start justify-between gap-2">
                                <a href="{{ route('tickets.show', $match['ticket']->id) }}" class="text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:underline leading-tight">
                                    #TK-0{{ $match['ticket']->id }} : {{ \Illuminate\Support\Str::limit($match['ticket']->subject, 30) }}
                                </a>
                                <span class="px-1.5 py-0.5 rounded text-[9px] font-extrabold uppercase {{ $match['similarity'] === 'Tinggi' ? 'bg-red-100 text-red-800 dark:bg-red-950/40 dark:text-red-300' : 'bg-orange-100 text-orange-800 dark:bg-orange-950/40 dark:text-orange-300' }}">
                                    {{ $match['similarity'] }}
                                </span>
                            </div>
                            ...
                            <div class="bg-white dark:bg-gray-800 p-2 rounded text-[10px] border border-gray-100 dark:border-gray-700 text-gray-700 dark:text-gray-300">
                    ...
                    @empty
                        <p class="text-xs text-gray-500 dark:text-gray-400 text-center py-4 italic">Belum menemukan kasus serupa.</p>
                    @endforelse
                </div>
            </div>
        </div>
